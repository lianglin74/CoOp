...
function create_svg_rect(rect, color) {
...
function add_svg_objects(svg, type_to_rects) {
    for (let t in type_to_rects) {
        for (let r = 0; r < type_to_rects[t].length; r++) {
            ...
            let svg_r = create_svg_rect(rect, color);
            ...
            let svg_t = create_svg_text(label, rect[0] + 1, rect[1] + 1, font_size, color);
...
function add_svg_image_divs(all_url, all_key, all_type_to_rects) {
    ...
    for (i = 0; i < all_url.length; i++) {
        ...
        elem.onclick = function(img_url, type_to_rects) {
            return function() {
                openViewer(img_url, type_to_rects);
            }
        } (all_url[i], all_type_to_rects[i]);
...
function update_svg_objects(svg, showlabel, showall, cls) {
    ...
    for (let rect of rects) {
        ...
        rect.setAttribute("visibility", box_visible ? "visible" : "hidden");
        ...
        t_label.setAttribute("visibility", box_visible && showlabel ? "visible" : "hidden");
        t_att.setAttribute("visibility", box_visible && showlabel ? "visible" : "hidden");
    }
}

function update_images(all_type_to_rects, showlabel, showall, cls) {
    for (i = 0; i < all_type_to_rects.length; i++) {
        let svg = document.getElementById('svg' + i);
        update_svg_objects(svg, showlabel, showall, cls);
    }
...
var resizeCallback = function() {
    ...
    var new_num_cols = w > 1000 ? 5 : w > 750 ? 4 : w > 500 ? 3 : w > 250 ? 2 : 1;
...
function openViewer(img_url, type_to_rects)
{
    let img = document.createElement("img");
...
function setup_menu() {
    ...
    document.title = "TSV - " + data + " - " + subset

    document.getElementById("show_label").checked = localStorage.getItem('show_label')
        ? JSON.parse(localStorage.getItem('show_label'))
        : false;;
    document.getElementById("show_all").checked = localStorage.getItem('show_all')
        ? JSON.parse(localStorage.getItem('show_all'))
        : false;;
}
