...
class InputInstance(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, score=None, img_key=None, q_id=None, boxes=None):
        """Constructs a InputExample.
        ...
        Args:
            ...
            label: (Optional) string. The label of the example. This should be
            specified for train and dev examples, but not for test examples.
            boxes: the precomputed regions to extract feature
        """
...
class VQATextProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, file_name='train2014_qla.json'):
        ...
        if file_name.endswith('_img.json'):
            return self._create_examples_from_img(lines, "train")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            boxes = None
            if 'boxes' in line:
                boxes = np.frombuffer(base64.b64decode(line['boxes']), np.float32).reshape((-1, 4))
            examples.append(InputInstance(guid=guid, text_a=text_a, text_b=text_b, label=label, score=score, img_key=img_key, q_id=q_id, boxes=boxes))
        return examples

    def _create_examples_from_img(self, images, set_type):
        """Creates examples for the training and dev sets."""

        examples = {}
        for (i, image) in enumerate(images):
            text_b = image['o']  # or None
            img_key = image['img_id']
            boxes = None
            if 'boxes' in image:
                boxes = np.frombuffer(base64.b64decode(image['boxes']), np.float32).reshape((-1, 4))
            examples_per_img = []
            for qa_ind, line in enumerate(image['qas']):
                if set_type!='test' and len(line['an']) == 0: continue
                guid = "%s-%s-%s" % (set_type, str(i), str(qa_ind))
                text_a = line['q']
                label = None if set_type.startswith('test') else line['an']
                score = None if set_type.startswith('test') else line['s']
                q_id = int(line['q_id']) if set_type.startswith('test') else 275*i+qa_ind
                examples_per_img.append(InputInstance(guid=guid, text_a=text_a, text_b=text_b, label=label, score=score, img_key=img_key, q_id=q_id, boxes=boxes))
            if len(examples_per_img) > 0:
                examples[img_key] = examples_per_img
        return examples
