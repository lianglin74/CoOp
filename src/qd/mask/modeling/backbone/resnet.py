...
from qd.mask.layers import Conv2d
from qd.mask.layers import DFConv2d
from qd.mask.modeling.make_layers import group_norm, frozen_batch_norm
...
class Bottleneck(nn.Module):
    def __init__(
        ...
        if use_se:
            self.seblock = SEBlock(out_channels, reduction_ratio=16)
...
class BaseStem(nn.Module):
    def __init__(self, cfg, norm_func):
        ...
        self.padding_mode = cfg.MODEL.RESNETS.STEM_PADDING_MODE
        if self.padding_mode == 'pad':
            self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        elif self.padding_mode == 'ceil':
            self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True)
        else:
            raise ValueError("ResNet STEM_PADDING_MODE {} not supported!"
