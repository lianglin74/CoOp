...
class ROIBoxHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None):
        ...
        if not self.return_distill_info:
            if isinstance(loss_classifier, dict):
                loss_info = dict(**loss_classifier, loss_box_reg=loss_box_reg)
            else:
                loss_info = dict(loss_classifier=loss_classifier, loss_box_reg=loss_box_reg)
            return (
                x,
                proposals,
                loss_info,
