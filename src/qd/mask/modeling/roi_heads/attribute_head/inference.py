...
class AttributePostProcessor(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #if "vgmergedattr524.labelmap.tsv" in cfg.DATASETS.ATTR_LABELMAP_FILE:
            ## merge blonde (38) to blond (37)
            #self.merge_list = {37: [38]}
            #print("for DATASETS.ATTR_LABELMAP_FILE = vgmergedattr524.labelmap.tsv, use the following merge list:")
    ...
    def filter_results(self, boxlist, prob, feature, num_classes):
        ...
        scores[:, 0] = 0.0
        # merge attribute based on merge list
        #if self.merge_list is not None:
            #for to_attr, from_attrs in self.merge_list.items():
                #scores[:, to_attr] = scores[:, to_attr] + torch.sum(scores[:, from_attrs], dim=1)
                #scores[:, from_attrs] = 0.0
