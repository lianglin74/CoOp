...
from ..mask_head.mask_head import keep_only_positive_boxes
...
class ROIAttributeHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.uselabelmap = cfg.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ALIGNED_CLASSES > 0
        if self.uselabelmap:
            self.register_buffer('alignedlabelmap',
                                 torch.load(cfg.MODEL.ROI_ATTRIBUTE_HEAD.ALIGNEDLABELMAP))
    ...
    def forward(self, features, proposals, targets=None):
        ...
        if self.training:
            ...
            # proposals here might be in cpu for faster post processing in box
            # head
            proposals = [b.to(features[0].device) for b in proposals]

        # deal with the case when len(proposals)==0 in the inference time; we assume that this will not happen in training
        ...
        if self.uselabelmap:
            labels = torch.cat(
                [self.alignedlabelmap[boxes_per_image.get_field("labels")].view(-1) for boxes_per_image in proposals], dim=0)
        else:
            labels = torch.cat(
