...
from qd.mask.layers.bert.modeling_utils import BeamHypotheses
...
class ConstrainedBeamSearch(object):
    ...
    def __init__(
        ...
        bad_ending_ids: Optional[List[int]] = None,
        decoding_constraint_flag: bool = False,
        use_hypo: bool = False,
        tokenizer=None,
    ):
        ...
        self.bad_ending_ids = bad_ending_ids
        self.decoding_constraint_flag = decoding_constraint_flag
    ...
    def search(
        ...
        for timestep in range(self.max_steps - curr_len - 1):
            ...
            #if (last_predictions == self._eos_token_ids).all():
                #break
            ...
            if self.decoding_constraint_flag:
                tmp = class_log_probabilities.new_zeros(class_log_probabilities.size())
                tmp.scatter_(-1, last_predictions.unsqueeze(-1).cuda(), float('-inf'))
                class_log_probabilities += tmp

            if self.bad_ending_ids:
                tmp = class_log_probabilities.new_zeros(class_log_probabilities.size())
                prev_bad = np.isin(last_predictions.cpu().numpy(), self.bad_ending_ids)
                prev_bad = torch.from_numpy(prev_bad).to(dtype=torch.bool)
                for eos_token_id in self._eos_token_ids:
                    tmp[prev_bad, eos_token_id] = float('-inf')
                class_log_probabilities += tmp
        ...
        if self.use_hypo:
            ...
            for batch_ex in range(batch_size):
                for i in range(num_fsm_states):
                    ...
                    # Hack to return sum_logprob
                    scores[batch_ex, i, 0] = score * len(sent)
...
class ConstraintBoxesReader(object):
    ...
    @classmethod
    def _parse_str_np_array(self, s):
