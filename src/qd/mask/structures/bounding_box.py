...
class BoxList(object):
    ...
    @classmethod
    def create_empty_list(image_size, mode='xyxy'):
        bbox = torch.tensor(0, 4)
        return BoxList(bbox, image_size, mode)
    ...
    def convert(self, mode):
        ...
        if mode == "xyxy":
            ...
            bbox = torch.cat(
                ((xmin + xmax + TO_REMOVE) / 2., (ymin + ymax + TO_REMOVE) / 2, xmax - xmin + TO_REMOVE, ymax - ymin + TO_REMOVE), dim=-1
            )
            bbox = BoxList(bbox, self.size, mode=mode)
