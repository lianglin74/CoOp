...
from mmrep import __version__
from mmrep.apis import set_random_seed, train_detector
from mmrep.datasets import build_dataset
from mmrep.models import build_detector
from mmrep.utils import collect_env, get_root_logger
from mmcv.parallel import MMDataParallel, MMDistributedDataParallel
from mmrep.apis import multi_gpu_test, single_gpu_test


class RepPointPipeline(MaskClassificationPipeline):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        if self.net.startswith('reppoints_v2'):
            folder = 'reppoints_v2'
        else:
            folder = 'reppoints_v1'
        config_file = op.join(
    ...
    def get_test_model(self):
        ...
        if fp16_cfg is not None:
            from mmrep.core import wrap_fp16_model
    ...
    def get_test_data_loader(self):
        cfg = self.cfg
        from mmrep.datasets import build_dataloader, build_dataset
        dataset = build_dataset(cfg.data.test)
        data_loader = build_dataloader(
            dataset,
            samples_per_gpu=1, # currently it has to be 1.
