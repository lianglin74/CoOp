...
class DarkNet(CNNModel):
    ...
    def add_body(self, netspec, depth=-1, lr=1, deploy=True):
        ...
        net_defs = {8:net_defs8,16:net_defs16, 19:net_defs19};
        for s in net_defs[depth]:
            if s[0].startswith('dark'):
                nout = s[1]
                self.dark_block(n, s[0], nout,  lr=lr, deploy=deploy)
    ...
    def add_extra(self, netspec, num_classes, lr=1, lr_lastlayer=1, deploy=True):
        n = netspec
        n.conv6f = conv(last_layer(n), nout=num_classes, ks=1, stride=1, pad=0, lr=lr, deploy=deploy, weight_filler=dict(type='xavier'));
