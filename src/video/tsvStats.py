...
def tsvStats(labelFileName, statFileName=None):
    ...
    total = 0
    labelCount = {'empty': 0, 'basketball': 0, 'rim': 0,
                  'backboard': 0,  'ball+rim': 0, 'all': 0}
    ...
    for k, v in labelCount.items():
        if statFileName is None:
            print("Count for class " + k + " is: " + str(v) +
                  ", percentage:" + str(v*100.0/total) + '\n')
        else:
            f.write("Count for class " + k + " is: " + str(v) +
                    ", percentage:" + str(v*100.0/total) + '\n')
