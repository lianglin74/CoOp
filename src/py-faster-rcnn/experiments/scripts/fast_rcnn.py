# Usage:
# python -u ./experiments/scripts/fast_rcnn.py ^
...
if __name__ == "__main__":
  ...
  if not args.LOG_TO_SCREEN:
    ...
    LOG = at_fcnn('experiments/logs/fast_rcnn_%s_%s_%s.txt' %
  ...
  train_net.main([
    ...
    '--imdb', TRAIN_IMDB,
    '--iters', str(ITERS),
    '--cfg', at_fcnn('experiments/cfgs/fast_rcnn.yml')] +
  ...
  test_net.main([
    '--gpu', args.GPU,
    '--def', at_fcnn('models/%s/%s/fast_rcnn/test.prototxt' % (PT_DIR, args.NET)),
    '--net', NET_FINAL,
    '--imdb', TEST_IMDB,
    '--cfg', at_fcnn('experiments/cfgs/fast_rcnn.yml')] + \
