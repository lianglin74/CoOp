...
def writeImagesWithLabels(topDir, labelFileName, video_name, startSecond, endSecond, writeImage = 0):
    ...
    #dir
    if writeImage: 
      directory = topDir + "/frames_" + video_name + "/" + "start_" + str(startSecond) + "-end_" + str(endSecond) + "/"
    ...
    while i <= endFrame:
      ...
      frame = cv2.putText(frame, text, (int(0), int(60)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
...
def drawLabel(image, labels):
  ...
  for v in labels:    
    ...
    image = cv2.putText(image, text, pos, cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
...
def skipRects(labels):
  ...
  for v1 in labels:
    #[{"conf": 0.7487, "obj": 0.7487, "class": "basketball", "rect": [1033.5602188110352, 38.45284080505371, 1070.4107284545898, 68.64767646789551]}, {"conf": 0.8385, "obj": 0.8385, "class": "basketball rim", "rect": [408.87592697143555, 251.28884315490723, 486.58501052856445, 312.7845211029053]}, {"conf": 0.9332, "obj": 0.9332, "class": "backboard", "rect": [355.62933349609375, 127.55272674560547, 458.9013671875, 302.18677520751953]}]
    skip = any([ rectInRect(v1['rect'], v2['rect']) for v2 in labels if v2 != v1 and v2['class'] == v1['class']])
    if not skip:
